...
static void
dhdpcie_check_reset_sysmem(dhd_bus_t *bus)
{
...
dhdpcie_dongle_attach(dhd_bus_t *bus)
{
	...
	*/
	 ...
	 */
	dhdpcie_check_reset_sysmem(bus);
		...
		case DUMP_TYPE_ESCAN_SYNCID_MISMATCH:
			...
			if (dhdp->db7_trap.fw_db7w_trap) {
				...
				if (dhdp->db7_trap.fw_db7w_trap_received) {
					 ...
					 */
#ifdef DHD_SSSR_DUMP
					DHD_PRINT(("%s : Set collect_sssr as TRUE after DB7\n",
						__FUNCTION__));
...
void dhd_bus_dump_flowring(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	/* flowring watermark stats */
	bcm_bprintf(strbuf, "\nFlowring unprocessed items stats:\n");
...
{
	si_core_cflags(dhd->bus->sih, SICF_CPUHALT, 0);
	return BCME_OK;
}
...
dhd_bus_get_ewp_etb_config(dhd_bus_t *bus)
{
	...
	ret = dhdpcie_bus_membytes(bus, FALSE, DHD_PCIE_MEM_BAR1,
			(ulong)ewp_info->etb_config_info_addr, (uint8 *)&etb_config_info,
	...
	ret = dhdpcie_bus_membytes(bus, FALSE, DHD_PCIE_MEM_BAR1,
			(ulong)ewp_info->etb_config_info_addr, (uint8 *)bus->etb_config_info, size);
	if (ret < 0) {
	...
	for (i = 0; i < etb_cfg->num_etb; ++i) {
		if (bus->etb_validity[i]) {
			...
			if (dhdp->dongle_trap_occured) {
				 ...
				 * config info
				 */
				rwpaddr = ewp_info->etb_config_info_addr +
					sizeof(etb_config_info_t) +
...
dhd_pcie_dump_wrapper_regs(dhd_pub_t *dhd)
{
	...
	if (CHIPTYPE(sih->socitype) != SOCI_NCI) {
		...
		/* size is in terms of 32-bit words */
